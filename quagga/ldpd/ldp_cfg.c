...
mpls_return_enum ldp_cfg_if_set(mpls_cfg_handle handle, ldp_if * i, uint32_t flag)
{
  ...
  if (flag & LDP_CFG_ADD) {
    ...
    if ((iff = ldp_global_find_if_handle(global, i->handle)) != NULL) {
      LDP_PRINT(global->user_data, "Duplicate interface exists");
      goto ldp_cfg_if_set_end;
    }
    if (mpls_if_handle_verify(global->ifmgr_handle, i->handle) ==
      MPLS_BOOL_FALSE) {
      LDP_PRINT(global->user_data, "Invalid interface handle");
      goto ldp_cfg_if_set_end;
    }
    if ((iff = ldp_if_create(global)) == NULL) {
      LDP_PRINT(global->user_data, "Failure creating interface");
      goto ldp_cfg_if_set_end;
    }
  ...
  ldp_addr *a, uint32_t flag)
{
  ...
  if (!iff) {
      LDP_PRINT(global->user_data, "ldp_cfg_if_addr_set: no such iff\n");
